...
describe('albatros', function() {
    ...
    describe('parsing sample 2014-07-14', function() {
        ...
        var html = fs.readFileSync(__dirname + '/samples/Albatros.2014-07-14.html', { encoding: "utf-8" });
        var menu;
        ...
        it("1st item correct", function() {
            assert.equal(menu[0].isSoup, true);
            assert.equal(menu[0].text, "Paradajková s cestovinou");
            assert.equal(menu[0].price, 1.10);
        });

        it("2nd item correct", function() {
            assert.equal(menu[1].isSoup, false);
            assert.equal(menu[1].text.trim(), "Kurací Cordon bleu, dusená ryža");
            assert.equal(menu[1].price, 3.20);
        });

        it("3rd item correct", function() {
            assert.equal(menu[2].isSoup, false);
            assert.equal(menu[2].text, "Bravčové karé na šampiňónoch, varené zemiaky");
            assert.equal(menu[2].price, 3.20);
        });

        it("4th item correct", function() {
            assert.equal(menu[3].isSoup, false);
            assert.equal(menu[3].text, "Zapekané cestoviny s brokolicou a syrom");
            assert.equal(menu[3].price, 3.00);
        });

        it("5th item correct", function() {
            assert.equal(menu[4].isSoup, false);
            assert.equal(menu[4].text, "Na objednávku (vyprážaný bravčový, kurací rezeň, syr)");
            assert.equal(menu[4].price, 3.20);
        });
