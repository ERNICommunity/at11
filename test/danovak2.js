...
describe("danovak2", function() {
    describe("parsing sample 2015-07-29", function() {
        ...
        it("5th item correct", function() {
            ...
            assert.equal(
                menu[4].text.trim(),
                "Pečené bravčové karé s kosťou špikované slaninkou a cibuľkou, varené zemiaky, kyslá uhorka"
            );
        ...
        it("8th item correct", function() {
            ...
            assert.equal(
                menu[7].text.trim(),
                "Dusená panenka na nivovej omáčke, 1/2 dusená ryža, 1/2 hrnolky, šalát alebo koláčik, kávička"
            );
            assert.equal(menu[7].price, 5.5);
        });
    });
});
